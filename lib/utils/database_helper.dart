...
class DatabaseHelper{
  ...
  initDb()async {
    ...
    String path = join(databasePath , 'empluoyees.db');
  ...
  void _onCreate(Database db , int newVersion) async{
    var sql = 'CREATE TABLE $tableEmployee ($columnId INTEGER PRIMARY KEY,'
        '$columnFullName TEXT ,$columnAdresse TEXT ,'
  ...
  Future<Employee> getEmployee(int id) async{
    ...
    List<Map> result = await dbClient.query(
        tableEmployee ,
        columns: [columnId,columnAdresse, columnFullName,columnDepartment,
  ...
  Future<int> updateEmployee2(Employee employee)async{
    var dbClient = await db;
    return await dbClient.update(
        tableEmployee , employee.toMap(), where:
         '$columnId = ?',whereArgs: [ employee.id ]
    );
  }
